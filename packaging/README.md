...
forka, jCI, jperm, distrACTION, scatr; jRISK osobno jako .jmo). Aplikacja = powłoka Electron + serwer Python +
...
- **Build Windows — instalator** — `build.ps1` przechodzi i produkuje `.jmo` (potwierdzone
  2026-08-26, macierz w `MODULES.md`). Instalator NSIS od 2026-09-11 jest MultiUser
  (per-user / dla wszystkich, `/S`, `/AllUsers`, `/D=`; opis w `30-distribution.md`):
  tryb per-user i cicha (de)instalacja sprawdzone na Windows 11; tryb „dla wszystkich"
  (UAC, `C:\Program Files`, HKLM) wymaga jeszcze ręcznego testu na koncie administratora.
