...
test_that("regression bootstrap drops failed coefficient pairs together", {
    d <- data.frame(x = c(0, 0, 0, 1), y = 1:4)
    expect_true(all(is.na(regressionBootStatistic(d, c(1, 2, 3, 1)))))
    r <- bootCI(d, regressionBootStatistic, 500, 1, "perc", .95, minValid = 250)
    expect_gt(r[[1]]$nFailed, 0)
    expect_identical(is.na(r[[1]]$reps), is.na(r[[2]]$reps))
    expect_true(r[[1]]$sufficient && r[[2]]$sufficient)
    band <- regressionBootBand(c(0, 1), coef(lm(y ~ x, d)), r[[1]]$reps, r[[2]]$reps, .95)
    expect_true(all(is.finite(as.matrix(band))))
    keep <- is.finite(r[[1]]$reps) & is.finite(r[[2]]$reps)
    expect_equal(band$lower[2], unname(quantile((r[[1]]$reps + r[[2]]$reps)[keep], .025)))
    empty <- bootCI(d, function(d, i) c(NA_real_, NA_real_), 100, 1, "bca", minValid = 50)
    expect_false(empty[[1]]$sufficient)
    expect_equal(empty[[1]]$nFailed, 100)
    expect_true(is.na(empty[[1]]$lower))
    few <- bootCI(d, regressionBootStatistic, 100, 1, "perc", minValid = 100)
    expect_false(few[[1]]$sufficient)
    expect_true(is.na(few[[1]]$lower))
})
