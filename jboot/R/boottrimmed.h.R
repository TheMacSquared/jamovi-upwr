...
bootTrimmedOptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "bootTrimmedOptions",
...
bootTrimmedResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "bootTrimmedResults",
    inherit = jmvcore::Group,
    active = list(
        trimTable = function() private$.items[["trimTable"]],
...
bootTrimmedBase <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "bootTrimmedBase",
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(0,1,0),
                options = options,
                results = bootTrimmedResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
#' \tabular{llllll}{
#'   \code{results$trimTable} \tab \tab \tab \tab \tab a table \cr
#'   \code{results$plots} \tab \tab \tab \tab \tab an array of images \cr
#' }
#'
#' Tables can be converted to data frames with \code{asDF} or \code{\link{as.data.frame}}. For example:
#'
#' \code{results$trimTable$asDF}
#'
#' \code{as.data.frame(results$trimTable)}
#'
#' @export
bootTrimmed <- function(
    data,
    dep,
    trimProp = 0.1,
    nBoot = 1000,
    ...
    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("bootTrimmed requires jmvcore to be installed (restart may be required)")
    ...
    options <- bootTrimmedOptions$new(
        dep = dep,
        trimProp = trimProp,
        nBoot = nBoot,
        ciWidth = ciWidth,
