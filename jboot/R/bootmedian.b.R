...
bootMedianClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    "bootMedianClass",
    inherit = bootMedianBase,
    private = list(
        .run = function() {
            ...
            for (depName in self$options$dep) {
                ...
                if (n < 2) {
                    ...
                    table$addFootnote(rowKey=depName, col="median",
                        "Za mało obserwacji")
                    next
                }

                bootFun <- function(data, i) median(data[i])
