Chi2DistributionClass <- if (requireNamespace('jmvcore')) R6::R6Class(
    ...
    private = list(
      ...
      .run = function() {
        ...
        if (ShowMean || ShowVariance) {
          ...
          if (ShowVariance) {
            MomentsTable$addRow(rowKey = "var", values = list(
              MomentColumn = "Var[X]",
              FormulaColumn = "Var[X] = 2 · (df + 2λ)",
              ValueColumn = as.character(round(2 * (DP1 + 2 * DP2), 4))
            ))
          }
        } else {
        ...
        # Shaded area under the density up to the quantile, so students
        # can see p as an area rather than just the dashed line
      ...
      .plot = function(image, ggtheme, theme, ...) {
        ...
        if (QuantileFunction=="TRUE") {
          Plot <- Plot+
            # The shaded area under the density up to the quantile
