WeibullDistributionClass <- if (requireNamespace('jmvcore')) R6::R6Class(
  "WeibullDistributionClass",
  inherit = WeibullDistributionBase,
  private = list(
    ...
    .run = function() {
      ...
      DP1 <- self$options$dp1  # shape (beta)
      DP2 <- self$options$dp2  # scale (eta)

      UpperTail <- qweibull(0.999, DP1, DP2)
      ...
      InputLabel1 <- paste("Kształt (β) = ", DP1, ", Skala (η) = ", DP2, sep = "")
      ...
      Density <- dweibull(x, DP1, DP2)
      Density[!is.finite(Density)] <- NA

      if(DistributionFunction == "TRUE"){
        if (DistributionFunctionType == "lower")
          DistributionResult <- pweibull(XValue, DP1, DP2)
        # upper tail computed directly for numerical stability
        if (DistributionFunctionType == "higher")
          DistributionResult <- pweibull(XValue, DP1, DP2, lower.tail = FALSE)
        if (DistributionFunctionType == "interval")
          DistributionResult <- pweibull(XValue2, DP1, DP2) - pweibull(XValue, DP1, DP2)}

      if(QuantileFunction == "TRUE"){
        if (QuantileFunctionType == "cumulative")
          QuantileResult <- qweibull(Quantile, DP1, DP2)
        if (QuantileFunctionType == "central"){
          QuantileResult <- qweibull(LowerQuantile, DP1, DP2)
          QuantileResult2 <- qweibull(HigherQuantile, DP1, DP2)}}
      ...
      if (ShowMean || ShowVariance) {
        ...
        if (ShowMean) {
          MomentsTable$addRow(rowKey = "mean", values = list(
            MomentColumn = "E[X]",
            FormulaColumn = "E[X] = η·Γ(1 + 1/β)",
            ValueColumn = as.character(round(DP2 * gamma(1 + 1/DP1), 4))
          ))
        }
        if (ShowVariance) {
          MomentsTable$addRow(rowKey = "var", values = list(
            MomentColumn = "Var[X]",
            FormulaColumn = "Var[X] = η²·[Γ(1 + 2/β) − Γ(1 + 1/β)²]",
            ValueColumn = as.character(round(DP2^2 * (gamma(1 + 2/DP1) - gamma(1 + 1/DP1)^2), 4))
          ))
        }
      } else {
      ...
      if(QuantileFunction == "TRUE"){
        if(QuantileFunctionType == "cumulative"){
          HigherSegment <- qweibull(Quantile, DP1, DP2)
          HigherSegmentLength <- dweibull(HigherSegment, DP1, DP2)
        ...
        if(QuantileFunctionType == "central"){
          LowerSegment <- qweibull(LowerQuantile, DP1, DP2)
          LowerSegmentLength <- dweibull(LowerSegment, DP1, DP2)
          HigherSegment <- qweibull(HigherQuantile, DP1, DP2)
          HigherSegmentLength <- dweibull(HigherSegment, DP1, DP2)
